...
def regrid(sourceimage,targetimage, savefile = True, resultimage = 'same', header = None, mode = 3):
    ...
    savefile : boolean
    	...
    	If False then returns the regrid image.
    	Default is True.	
    resultimage : string
    ...
    header : header format or None
    	...
    	If header is given, then that is used for result header.
    	Default is None. 
    mode : integer 
    ...
    tofill : 2-D array 
    	sourceimage regrid to the shape and size of targetimage and return as a 2D array.	      

    ~~~~~ By: Natalie Price-Jones and Ayushi Singh~~~~~
   	"""
	# Start timer
	start = time.time()
