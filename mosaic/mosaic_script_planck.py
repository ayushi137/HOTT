...
file_type = 'without_plane'
suffix = '_1'

data = '2020-07-15'

offset_file = main_folder+'/offset_values/Offset_and_mu_{1}{0}.csv'.format(suffix,date)
...
for i in names:
    if frequency:
        a = glob.glob(main_folder+'offset_maps/{0}*160.{1}.fits'.format(i,file_type))
        b = glob.glob(main_folder+'offset_maps/{0}*250.{1}.fits'.format(i,file_type))
        c = glob.glob(main_folder+'offset_maps/{0}*350.{1}.fits'.format(i,file_type))
        d = glob.glob(main_folder+'offset_maps/{0}*500.{1}.fits'.format(i,file_type))
    ...
    g = glob.glob(main_folder+'grid/{0}_planck.fits'.format(i))
    if SED:
        x = glob.glob(main_folder+'tau_temp/{0}{1}/{0}_tau1THz*.fits'.format(i, suffix))
        y = glob.glob(main_folder+'tau_temp/{0}{1}/{0}_dustTemperature*.fits'.format(i, suffix))
        z = glob.glob(main_folder+'tau_temp/{0}{1}/{0}_H2columnDensity*.fits'.format(i, suffix))
...
# load template 
'''
template, head = fits.getdata(planck_orig_temp, header = True)
...
for i in range(file_num):
    ...
    if frequency:
        ...
        data_160[i]= fits.getdata(data_160_names[i]) + offset_val[0]
        ...
        data_250[i]= fits.getdata(data_250_names[i]) + offset_val[1]
        ...
        data_350[i]= fits.getdata(data_350_names[i]) + offset_val[2]
...
planck_temp = fits.getdata(planck_temp_n)
planck_beta = fits.getdata(planck_beta_n)

if SED:
    planck_colden = fits.getdata(planck_colden_n)
    planck_tau = fits.getdata(planck_tau_n)

if frequency:
    planck_160 = fits.getdata(planck_160)
    if apply_cc == True:
        print ('Start the colour correction script.')
        planck_160 = applying_cc(planck_160,160, planck_temp, planck_beta)    
        print ('Finish colour correcting Planck data.\n')

    planck_250 = fits.getdata(planck_250)
    if apply_cc == True:
        print ('Start the colour correction script.')
        planck_250 = applying_cc(planck_250,250, planck_temp, planck_beta)    
        print ('Finish colour correcting Planck data.\n')

    planck_350 = fits.getdata(planck_350)
    if apply_cc == True:
        print ('Start the colour correction script.')
        planck_350 = applying_cc(planck_350,350, planck_temp, planck_beta)    
        print ('Finish colour correcting Planck data.\n')

    planck_500 = fits.getdata(planck_500)
    if apply_cc == True:
        print ('Start the colour correction script.')
